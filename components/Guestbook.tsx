...
const Guestbook: React.FC = () => {
  ...
  const [copiedIndex, setCopiedIndex] = useState<{ type: 'groom' | 'bride', index: number } | null>(null);
    ...
    `}>
      <div className="px-6 max-w-xl mx-auto space-y-6">
        {accounts.map((acc, index) => (
          <div key={index} className="flex flex-col md:flex-row md:items-center justify-between p-4 border border-white/5 hover:border-white/20 transition-colors bg-[#111] relative group">
             {/* Decorative corners */}
             ...
             <div className="mb-4 md:mb-0">
               <div className="text-[10px] text-gray-500 uppercase tracking-widest mb-1">{acc.bank}</div>
               <div className="text-lg text-gray-200 font-cinematic tracking-wide">{acc.accountNumber}</div>
    ...
    <section className="bg-[#111111] py-24 border-t border-white/5">
       <div className="max-w-2xl mx-auto px-6 mb-12">
        ...
        <p className="text-sm text-gray-500 font-light leading-relaxed">
          참석하지 못하시는 분들을 위해 계좌번호를 안내해 드립니다.<br/>
      ...
      <div className="border-t border-white/10 border-b border-white/10">
        ...
        >
          ...
          `} style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=1974&auto=format&fit=crop)' }}></span>
          
          <div className="relative z-10 flex flex-col items-center">
             <span className="text-[10px] tracking-[0.4em] text-gray-400 uppercase mb-2 group-hover:text-white transition-colors">Groom's Side</span>
             <span className="text-2xl md:text-3xl font-cinematic text-white">신랑측 계좌번호</span>
          </div>
          
          <div className={`absolute right-8 transition-transform duration-300 ${activeTab === 'groom' ? 'rotate-180' : ''}`}>
             <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M19 9l-7 7-7-7" /></svg>
          </div>
        </button>
        ...
        >
          ...
          `} style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=2070&auto=format&fit=crop)' }}></span>
          
          <div className="relative z-10 flex flex-col items-center">
             <span className="text-[10px] tracking-[0.4em] text-gray-400 uppercase mb-2 group-hover:text-white transition-colors">Bride's Side</span>
             <span className="text-2xl md:text-3xl font-cinematic text-white">신부측 계좌번호</span>
          </div>

          <div className={`absolute right-8 transition-transform duration-300 ${activeTab === 'bride' ? 'rotate-180' : ''}`}>
             <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M19 9l-7 7-7-7" /></svg>
          </div>
        </button>
